...
func handler(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		...
		token := passwordreset.NewToken(emailstruct.Event.Data.New.Email, 12*time.Hour, []byte(emailstruct.Event.Data.New.SecretToken), []byte(os.Getenv("secret")))

		url := fmt.Sprintf("%s/reset?token=%s", os.Getenv("MANGA_DOMAIN_URL"), token)
		err = email.SendWelcomeEmail(v.Email, v.Username, url)
		if err != nil {
			log.Println(err)
		}
	}
