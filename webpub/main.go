...
// 		Name:       v.Series.Name,
// 		Identifier: fmt.Sprintf("%s/series/%d/%s", beego.AppConfig.String("APP_LINK"), v.Series.Id, slugify.Slugify(v.Series.Name)),
// 	})
...
// 			link.Link = fmt.Sprintf("%s%s/%s", beego.AppConfig.String("cdn_url"), hash, k.UUID)
// 		} else {

// 			link.Link = fmt.Sprintf("%schapters/%s/%s", beego.AppConfig.String("cdn_url"), v.Hash, k.UUID)
// 		}
...
// 		Relation: "self",
// 		Link:     fmt.Sprintf("%s/series_chapters/%d", beego.AppConfig.String(""), v.Id),
